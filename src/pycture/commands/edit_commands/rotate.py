...
class Rotate(Command):
    ...
    def rotate(self, image: Image, angle_deg: float):
        ...
        new_top_left = np.array([floor(value) for value in np.dot(dt_rotation_matrix, old_top_left)]) 
        ...
        for indexXp in range(new_width):
            for indexYp in range(new_height):
                xp, yp = [floor(value) for value in (indexXp + min_x, indexYp + min_y)]
                (x, y) = [floor(value) for value in np.dot(it_rotation_matrix, (xp, yp))]
                if (0 <= x < image.width() and 0 <= y < image.height()):
                    new_image.setPixel(indexXp, indexYp, image.pixel(floor(x), floor(y)))
                else:
    ...
    def execute(self, main_window: QtWidgets.QMainWindow):
        ...
        rotated_image = self.rotate(image, -90)
        main_window.add_editor(editor=Editor(main_window, rotated_image, title + ' rotated 45º'))
